...
-->
	<TABLE BGCOLOR="{$DS_COL_BLACK}" WIDTH="{$table_width}" COLS="5" cellspacing="1" cellpadding="2" border="0">
		...
		<xsl:for-each select="MEMORYMAP/RANGE">
			<xsl:sort data-type="number" select="@BVALUE" order="ascending"/>
			<TR></TR>
				<xsl:variable name="isdata">
					<xsl:if test="@ADDR_ISDATA='TRUE'">&#9632;</xsl:if>	
					<xsl:if test="not(@ADDR_ISDATA='TRUE')">&#160;</xsl:if>	
				</xsl:variable>
			
			<xsl:variable name="isinst">
				<xsl:if test="@ADDR_ISINST='TRUE'">
					&#9632;
				</xsl:if>	
				<xsl:if test="not(@ADDR_ISINST='TRUE')">
					&#160;
				</xsl:if>	
			</xsl:variable>
