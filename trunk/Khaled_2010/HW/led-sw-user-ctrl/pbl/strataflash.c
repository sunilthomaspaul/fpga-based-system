...
#ifdef DEBUG_FLASH
    printf("flash_erase(0x%p, first: %d last: %d\n", info,  s_first, s_last);
#endif
...
#ifdef DEBUG_FLASH
    printf("write_buff(0x%p, src: 0x%p addr: 0x%x cnt: 0x%x\n", info, src, addr, cnt);
#endif
...
#ifdef DEBUG_FLASH
    printf("flash_real_protect(0x%p, sector: 0x%x prot: %d\n", info, sector, prot);
#endif
...
static void flash_write_cmd(flash_info_t * info, int sect, uchar offset, uchar cmd)
{
	...
	case FLASH_CFI_8BIT:
		*addr.cp = cword.c;
#ifdef DEBUG_FLASH
    printf("flash_write_cmd: 0x%02x @ 0x%x\n", cword.c, addr.cp);
#endif
		break;
	case FLASH_CFI_16BIT:
		*addr.wp = cword.w;
#ifdef DEBUG_FLASH
    printf("flash_write_cmd: 0x%04x @ 0x%x\n", cword.w, addr.wp);
#endif
		break;
	case FLASH_CFI_32BIT:
		*addr.lp = cword.l;
#ifdef DEBUG_FLASH
    printf("flash_write_cmd: 0x%08x @ 0x%x\n", cword.l, addr.lp);
#endif
		break;
	}
}
...
#ifdef DEBUG_FLASH
    printf("flash_isequal: does 0x%02x (read from 0x%p) == 0x%02x?\n", cptr.cp, cptr.cp[0], cword.c);
#endif
		break;
	case FLASH_CFI_16BIT:
		retval = (cptr.wp[0] == cword.w);
#ifdef DEBUG_FLASH
    printf("flash_isequal: does 0x%04x (read from 0x%p) == 0x%04x?\n", cptr.wp, cptr.wp[0], cword.w);
#endif
		break;
	case FLASH_CFI_32BIT:
		retval = (cptr.lp[0] == cword.l);
#ifdef DEBUG_FLASH
    printf("flash_isequal: does 0x%08x (read from 0x%p) == 0x%08x?\n", cptr.lp, cptr.lp[0], cword.l);
#endif
		break;
	default:
		retval = 0;
...
#ifdef DEBUG_FLASH
    printf("flash_isset: is 0x%02x (read from 0x%p) == 0x%02x?\n", cptr.cp[0], cptr.cp, cword.c);
#endif
	return retval;
}
...
#ifdef DEBUG_FLASH
    printf("flash_get_size(base: 0x%x, bank: %d)\n", base, banknum);
#endif
...
#ifdef DEBUG_FLASH
        printf("FLASH_OFFSET_ERASE_REGIONS: 0x%x \n", tmp);
#endif
			erase_region_count = (tmp & 0xffff) +1;
#ifdef DEBUG_FLASH
        printf("erase_region_count: %d \n", erase_region_count);
#endif
			tmp >>= 16;
			erase_region_size = (tmp & 0xffff)? ((tmp & 0xffff) * 256): 128;
#ifdef DEBUG_FLASH
        printf("erase_region_size: %d \n", erase_region_size);
#endif
...
#ifdef DEBUG_FLASH
        printf("FLASH_OFFSET_SIZE: 0x%x \n", flash_read_uchar(info, FLASH_OFFSET_SIZE));
#endif
