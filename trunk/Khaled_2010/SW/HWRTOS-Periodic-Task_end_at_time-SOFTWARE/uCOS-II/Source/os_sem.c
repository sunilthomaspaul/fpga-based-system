...
INT8U  OSSemCreate (INT16U cnt)                            // SW/HW RTOS                                              
{
    ...
    if (OSIntNesting > 0) {                                /* See if called from ISR ...               */        
        return (0);                                        /* ... can't CREATE from an ISR             */  
    }      
    Reg64Value_o.Lower |= cnt;                           
    Reg64Value_o.Lower = (Reg64Value_o.Lower << 8) | HWRTOS_SEMA4_CREATE; 
    OS_ENTER_CRITICAL();   
    HWRTOS_WriteSlaveReg0(hwrtos_addrp, &Reg64Value_o);       
    HWRTOS_ReadSlaveReg4(hwrtos_addrp, &Reg64Value_i);                
    OS_EXIT_CRITICAL();    
    pevent = (Reg64Value_i.Lower & 0x0000FF00) >> 8;
    return (pevent);
}
...
*/
/*$PAGE*/
void  OSSemPend (INT8U pevent, INT16U timeout, INT8U *perr)           // SW/HW RTOS                                  
{    
    Xuint64 Reg64Value_o;
    Xuint64 Reg64Value_i;  
    ...
    Reg64Value_o.Lower = (Reg64Value_o.Lower << 8) | HWRTOS_SEMA4_PEND; 
    OS_ENTER_CRITICAL();   
    HWRTOS_WriteSlaveReg0(hwrtos_addrp, &Reg64Value_o);       
    HWRTOS_ReadSlaveReg4(hwrtos_addrp, &Reg64Value_i);               
    //OS_EXIT_CRITICAL();
    if ((Reg64Value_i.Lower & 0x00010000) == 0x00010000) {                                
           ...
           OS_EXIT_CRITICAL();  
           *perr = (Reg64Value_i.Lower & 0x0000FF00) >> 8;
           return; 
    }        
    Reg64Value_i.Lower = 0;  
    Reg64Value_i.Upper = 0;
    Reg64Value_o.Lower = 0;                           
    Reg64Value_o.Lower |= pevent;    
    Reg64Value_o.Lower = (Reg64Value_o.Lower << 8) | HWRTOS_SEMA4_PEND_STAT; 
    OS_ENTER_CRITICAL();   
    HWRTOS_WriteSlaveReg0(hwrtos_addrp, &Reg64Value_o);       
    HWRTOS_ReadSlaveReg4(hwrtos_addrp, &Reg64Value_i);              
    OS_EXIT_CRITICAL();    
...
INT8U  OSSemPost (INT8U pevent)                       // SW/HW RTOS                                                   
{
...
#endif                                     
    Reg64Value_o.Lower |= pevent;    
    Reg64Value_o.Lower = (Reg64Value_o.Lower << 8) | HWRTOS_SEMA4_POST;
    OS_ENTER_CRITICAL();    
    HWRTOS_WriteSlaveReg0(hwrtos_addrp, &Reg64Value_o);             
    HWRTOS_ReadSlaveReg4(hwrtos_addrp, &Reg64Value_i);                     
    //OS_EXIT_CRITICAL();      
    if ((Reg64Value_i.Lower & 0x00010000) == 0x00010000) {                 
           ...
           OS_TASK_SW();   
           OS_EXIT_CRITICAL(); 
           return (OS_ERR_NONE);                      //return from function will enable interrupt
    }  
    else { 
           OS_EXIT_CRITICAL();          
           return((Reg64Value_i.Lower & 0x0000FF00) >> 8);            
    }                 
}
