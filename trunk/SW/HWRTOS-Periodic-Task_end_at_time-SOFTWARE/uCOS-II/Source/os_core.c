...
void  OSIntExit (void)                                    // SW/HW RTOS                                               
{
    ...
    OS_CPU_SR  cpu_sr = 0;
        ...
        if (ctx_sw == 0) {
           Reg64Value_o.Lower |= 0x01;                     // SW/HW RTOS: No ctx sw
           Reg64Value_o.Lower = (Reg64Value_o.Lower << 8) | HWRTOS_SEL_INT_OUT;                 
           HWRTOS_WriteSlaveReg0(hwrtos_addrp, &Reg64Value_o);               
           HWRTOS_ReadSlaveReg4(hwrtos_addrp, &Reg64Value_i);                
        }
        else {                                             // SW/HW RTOS: Ctx sw if HW required
           Reg64Value_o.Lower |= HWRTOS_SEL_INT_OUT;                              
           HWRTOS_WriteSlaveReg0(hwrtos_addrp, &Reg64Value_o);                                       
           HWRTOS_ReadSlaveReg4(hwrtos_addrp, &Reg64Value_i);                                  
           if (!(((Reg64Value_i.Lower & 0x0000FF00) == 0) & ((Reg64Value_i.Lower & 0x00FF0000) == 0))) {                                                     
               OSCurPrioStkPtr = &OSStkPtr[(Reg64Value_i.Lower & 0x0000FF00) >> 8];
               OSHiPrioStkPtr = &OSStkPtr[(Reg64Value_i.Lower & 0x00FF0000) >> 16];
               OSIntCtxSw();                                                              
           }                          
        }
        OS_EXIT_CRITICAL();           
...
void  OSStart (void)                                    // SW/HW RTOS                                                 
{
      ...
      Reg64Value_o.Lower |= HWRTOS_OS_START;                       
      HWRTOS_WriteSlaveReg0(hwrtos_addrp, &Reg64Value_o);                       
      HWRTOS_ReadSlaveReg4(hwrtos_addrp, &Reg64Value_i);                                
      if ((Reg64Value_i.Lower & 0x00000100) == 0) {
         OSHiPrioStkPtr = &OSStkPtr[(Reg64Value_i.Lower & 0x00FF0000) >> 16];
         OSCurPrioStkPtr = OSHiPrioStkPtr;                                
         OSStartHighRdy();
      }                                
}
/*$PAGE*/
...
#if OS_TASK_STAT_EN > 0
void  OSStatInit (void)                          // SW/HW RTOS                                                        
{
#if OS_CRITICAL_METHOD == 3                      /* Allocate storage for CPU status register           */
    OS_CPU_SR  cpu_sr = 0;
#endif
...
static  void  OS_InitMisc (void)                           // SW/HW RTOS                                              
{
    Xuint64 Reg64Value_o; 
    ...
    OSCurPrioStkPtr = (OS_STK **)0;
    OSHiPrioStkPtr = (OS_STK **)0;
   ...
   Reg64Value_o.Lower = HWRTOS_CMD_RST;                       
   HWRTOS_WriteSlaveReg0(hwrtos_addrp, &Reg64Value_o);      
   Reg64Value_o.Upper |= (HWRTOS_PLB_CLK_FREQ / HWRTOS_TICKS_PER_SEC);     //32 bits for HW TimerValue
   Reg64Value_o.Lower |= OS_TASK_IDLE_PRIO;
   Reg64Value_o.Lower = (Reg64Value_o.Lower << 8) | HWRTOS_HW_INIT;        
   HWRTOS_WriteSlaveReg0(hwrtos_addrp, &Reg64Value_o);          
}
/*$PAGE*/
/*
...
static  void  OS_InitTaskIdle (void)                                 // SW/HW RTOS                                    
{           
    ...
    (void)OSTaskCreatePeriodic(OS_TaskIdle,
                          ...
                          0x49,
                          0x0000);                                   //Character 'I' as idle task name                               
...
#if OS_TASK_STAT_EN > 0
static  void  OS_InitTaskStat (void)                                 // SW/HW RTOS                                    
{
#if OS_TASK_NAME_SIZE > 7
    INT8U  err;
#endif
    ...
    (void)OSTaskCreatePeriodic(OS_TaskStat,
                          ...
                          0x53,
                          0x0000);                                     //Character 'S' as statistic task name 
...
{         
    T3_cont_tmr_value = BSP_CnotTmrRead();        
#if OS_CRITICAL_METHOD == 3                      /* Allocate storage for CPU status register           */
    OS_CPU_SR  cpu_sr = 0;
#endif
    INT32U test;
           
    xil_printf("\r\n Task 1 end at = %lX \r\n", T1_cont_tmr_value - overhead);
    xil_printf("\r\n Task 2 end at = %lX \r\n", T2_cont_tmr_value - overhead);  
    xil_printf("\r\n Task 3 end at = %lX \r\n", T3_cont_tmr_value - overhead);
    xil_printf("\r\n -------------------------- \r\n");        
    test = getnum();
...
#if OS_TASK_STAT_EN > 0
void  OS_TaskStat (void *p_arg)                  // SW/HW RTOS                                                        
{
#if OS_CRITICAL_METHOD == 3                      /* Allocate storage for CPU status register           */
    OS_CPU_SR  cpu_sr = 0;
#endif
