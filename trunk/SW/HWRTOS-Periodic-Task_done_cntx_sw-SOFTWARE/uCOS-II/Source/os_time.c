...
void  OSTaskDone (void)                          // SW/HW RTOS                                                        
{
    ...
    if (OSIntNesting > 0) {                      /* See if trying to call from an ISR                  */
        ...
        Reg64Value_o.Lower |= HWRTOS_TASK_DONE; 
        OS_ENTER_CRITICAL();   
        HWRTOS_WriteSlaveReg0(hwrtos_addrp, &Reg64Value_o);          
        HWRTOS_ReadSlaveReg4(hwrtos_addrp, &Reg64Value_i);                                
        if ((Reg64Value_i.Lower & 0x00000100) == 0x00000100) {                                           
           OSCurPrioStkPtr = &OSStkPtr[(Reg64Value_i.Lower & 0x00FF0000) >> 16];
           OSHiPrioStkPtr = &OSStkPtr[(Reg64Value_i.Lower & 0xFF000000) >> 24];
           OS_TASK_SW();                        //return from function will enable interrupt
        }                        
        OS_EXIT_CRITICAL();         
}
/*$PAGE*/
